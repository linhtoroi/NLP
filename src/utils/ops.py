...
def merge_padded_seq_3D(hiddens1, masks1, hidden2, masks2):
    # hiddens1: text_hiddens Chính là hidden của layer cuối cùng của text qua LSTM 
    # Ví dụ kích thước là [4:28:400]
    # mask1: text_masks Chính là mask của text, phần nào False thì là câu text, True là phần padding cần mask 
    # Ví dụ kích thước là [4:28]
    # hidden2: values_embedded Chính là encoder_base_hidden là hidden của layer cuối cùng của text qua LSTM, sau đó được đi qua 1 bước batch_binary_lookup_3D 
    # Ví dụ kích thước [4:42:400]
    # mask2: values_masks Chính la mask của phần trên, padding thì là True, phần câu là False
    # Ví dụ kích thước [4:42]

    batch_size = len(hiddens1) # Ví dụ: 4
    # Tìm độ lớn của câu trong text_len (ko tính padding) 
    seq_len1 = masks1.size(1) - masks1.sum(dim=1) #Ví dụ [28,  9, 28, 14]
    # Tìm độ lớn của câu trong input_seq_len (ko tính padding) 
    seq_len2 = masks2.size(1) - masks2.sum(dim=1) #Ví dụ [42, 42, 35, 36]
    # Cộng size của text và input_seq (ko tính padding) 
    merged_size = seq_len1 + seq_len2 #Ví dụ [70, 51, 63, 50]
    # Tìm kích thước lớn nhất của merged_size
    max_merged_size = int(merged_size.max()) # Ví dụ 70
    # Độ dài lớn nhất của vector merge hidden text và input_seq trừ cho độ dài của text_len
    res1 = max_merged_size - hiddens1.size(1) # Ví dụ: 70 - 28 = 42
    # Concat text_hiddens và values_embedded lại, nếu nhỏ hơn max_merged_size thì padding thêm 0 phía sau
    merged_hiddens = torch.cat([hiddens1, zeros_var_cuda([batch_size, res1, hiddens1.size(2)])], dim=1) # Ví dụ kích thước [4:70:400]
    # Tạo ra ma trận có kích thước batch_size*batch_size = 4*4
...
def batch_binary_lookup_3D(M, b_idx, pad_value):
    ...
    """
    # Pad binary indices
    # M là ma trận hidden của cả câu input (bao gồm cả text và tên bảng tên cột phía sau) qua BiLSTM
    # Ví dụ kích thước [4:142:400]
    # b_idx là ma trận schema_mask với True là vị trí token đánh dấu sau là bảng [unused50] hoặc cột [unused51]
    # False là text, tên bảng, tên cột
    # The 1s in each row in idx indicates the features to be selected from M.
    # =>> nghĩa là sẽ lấy hidden của các token đánh dấu bảng, cột
    # Ví dụ kích thước [4:142]
    batch_size = M.size(0) # 4
    hidden_dim = M.size(2) # 400
    seq_len = b_idx.sum(1, keepdim=True) # [42, 42, 42, 42]
    # Tìm seq_len = tổng b_idx (tổng các True)
    max_seq_len = int(seq_len.max()) # 42
    # Ma trận kích thước 4*42 để False nếu phần đấy vẫn nhỏ hơn kích thước của câu hỏi đó, True nếu phần đó dài hơn câu hỏi
    # Mask đánh dấu phần padding, ví dụ khi ta lấy schema của bảng dài nhất, vector schema bảng ngắn hơn sẽ padding thêm 12 số 0 đằng sau, nên mask sẽ là 12 True phía sau
    output_masks = batch_arange_cuda(batch_size, max_seq_len) >= seq_len # kích thước [4:42]
    # Tính chiều dài phần padding vào mỗi câu
    pad_len = max_seq_len - seq_len # [0, 0, 0, 0]
    max_pad_len = int(pad_len.max()) # 0
    # Concat thêm vào cuối M max_pad_len giá trị 0 
    M = torch.cat([M, fill_var_cuda([batch_size, max_pad_len, hidden_dim], pad_value, dtype=M.dtype)], dim=1) # Kích thước [4:142:400]
    # Đánh dấu phần padding thêm, phần số 0 concat thêm vào cuối M nhãn True
    pad_b_idx = batch_arange_cuda(batch_size, max_pad_len) < pad_len 
    # Concat mask của schema_mask với phần nhãn phần padding thêm
    b_idx = torch.cat([b_idx, pad_b_idx], dim=1)
    # Lấy những phần trong M có nhãn mask là True
    # Giữ chiều (batch_size, max_seq_len, hidden_dim)
    output = M[b_idx].view(batch_size, max_seq_len, hidden_dim) # Kích thước [4:42:400]
    return output, output_masks #trả về hidden của phần schema và mask đánh dấu schema và phần padding
